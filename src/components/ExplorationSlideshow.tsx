...
export default function ExplorationSlideshow({ isActive, onNext, onReverse }: ExplorationSlideshowProps) {
    ...
    return (
        ...
        >
            ...
            {/* Large Left Title */}
            <div className={`absolute top-1/2 left-12 -translate-y-1/2 z-10 pointer-events-none select-none transition-opacity duration-300 ${focusedCardId ? 'opacity-20' : 'opacity-100'}`}>
                <h1 className="text-[12vw] font-bold text-white/10 leading-none tracking-tighter mix-blend-overlay">
                    {activeSlide.title}
                </h1>
                <h2 className="text-4xl font-bold text-white ml-2 tracking-widest mt-[-2vw]">
                    {activeSlide.title}
                </h2>
            </div>
            ...
            <div className="absolute bottom-12 right-12 z-40 flex items-center gap-6 pointer-events-auto">
            ...
            {/* Technical Overlay */}
            <div className="absolute top-8 left-8 z-30 font-mono text-xs text-white/50 pointer-events-none select-none">
